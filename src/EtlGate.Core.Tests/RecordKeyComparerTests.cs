		...
		public class When_asked_to_compare
		{
			...
			public void FuzzTestIt()
			{
				...
				for (var i = 0; i < 50000; i++)
				{
					...
					{
						var fieldName = fieldNames[j].ToString();
			...
			{
				var fields = Enumerable.Range(0, random.Next(10)).Select(x => possibleFieldNames[x].ToString()).Distinct().ToList();
			...
			public class LeftBeforeRightResultComparer : IFieldComparer
			{
			...
			public class RightBeforeLeftResultComparer : IFieldComparer
			{
			...
			public class SameResultComparer : IFieldComparer
			{
